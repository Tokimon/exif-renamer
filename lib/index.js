...
function renameFiles(files, cb, i = 0) {
  return new Promise((resolve, reject) => {
    ...
    if (!file) {
      ...
      return i >= files.length ? resolve(files) : renameFiles(files.splice(
        i, 1), cb, i).then(resolve);
    }
    ...
    newName(file).then((newpath) => {
      fs.rename(file.path, newpath, (err) => {
        if (err) {
          return console.warn(
            `Couldn't rename: ${files.splice(i--, 1).path} - skipping file`
          );
        }
...
function newName(filedata) {
  ...
  const datename = moment(`${date[0].replace(/[:]+/g, '-')} ${date[1]}`).format(
    args.name);
  return uniqueName(datename, n_path.extname(filedata.path), n_path.dirname(
    filedata.path));
}
...
const args = cmdArgs([{
  ...
  name: 'exp',
  type: String,
  defaultValue: '^\\d{4}(-\\d{2}){2} - (\\d{2})(\\.\\d{2}){2}(_\\d+)?\\.[^.]+$'
}, {
  name: 'ext',
  type: String,
  multiple: true,
...
allFileDatas(n_path.join(args.path, `/**/*.@(${args.ext.join('|')})`))
  ...
  .then((files) => {
    if (!files.length) {
      throw 'No files matched the filter expression: ' + args.exp;
    }
