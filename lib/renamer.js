...
const args = yargs
  ...
  .option('fsdate', {
    type: 'boolean',
    describe: 'Fallback to File System date if Exif information is not available'
  })
  .help()
  .alias('help', 'h')
  .argv;
...
function *renameFile(filename, list) {
  ...
  try {
    const date = yield fileDate(fileData.file, args.fsdate);
    ...
    fileData.newpath = nPath.join(dir, name.toLowerCase());
