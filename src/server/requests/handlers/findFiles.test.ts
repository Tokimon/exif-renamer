...
describe('server/requests/handlers/findFiles', () => {
  ...
  test('Finds the files from the given folder', async () => {
    ...
    expect(mockedGlob).toHaveBeenCalledWith(fileGlobExpression('root', xmpExtensions));
  });
  ...
  test('Finds only files with the given extensions', async () => {
    ...
    expect(mockedGlob).toHaveBeenCalledWith(fileGlobExpression('root', ext));
  });
  ...
  test('`shallow` = true, finds files at the given root only', async () => {
    ...
    expect(mockedGlob).toHaveBeenCalledWith(fileGlobExpression('root', ext, true));
  });
});
